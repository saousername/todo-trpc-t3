    ...
    return (
        <div className="flex flex-col items-center justify-start w-full">
            ...
            <div className="flex flex-row justify-center w-full gap-2.5">
                <form onSubmit={addNewTodo} className="grid w-full max-w-sm items-center gap-1.5">
                    <Label htmlFor="email">New Item</Label>
                    <>
                        <Input value={newTodoInput} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewTodoInput(e.target.value)} placeholder="e.g Complete challenge for Simeon" />
                        <Button type="submit">
                            Create
                        </Button>
                    </>
            ...
            <div className="flex flex-col items-center w-full max-w-[400px] py-4 gap-2">
                {!isLoading && !isError && data && data.map((todo) => 
                    <TodoItem key={todo.todo_id} todo={todo} toggleChecked={toggleChecked} />
                )}
            </div>
        </div>
    )
...
function TodoItem({ todo, toggleChecked }: { todo: ITodo, toggleChecked: (todo: ITodo) => void }) {
    return (
        <div className="flex flex-row items-center justify-start gap-2 w-full bg-gray-50 p-2 rounded-sm">
            <Checkbox checked={todo.is_checked ?? false} onCheckedChange={() => toggleChecked(todo)} />
